...
from Fubuki.core.call import Fubuki
...
from Fubuki.utils.inline.playlist import close_markup
from Fubuki.utils.pastebin import Fubukibin
from Fubuki.utils.stream.queue import put_queue, put_queue_index
from Fubuki.utils.thumbnails import gen_qthumb, gen_thumb
...
async def stream(
    ...
    import logging
    slog = logging.getLogger("Fubuki.utils.stream.stream")
    st0 = _time.monotonic()
    slog.info(
    ...
    if streamtype == "playlist":
        ...
        if streamtype == "saavn_track":
            ...
            if await is_active_chat(chat_id):
                ...
                if not forceplay:
                    db[chat_id] = []
                await Fubuki.join_call(chat_id, original_chat_id, file_path, video=None)
                await put_queue(
                    chat_id,
                    original_chat_id,
                    file_path,
        ...
        if await is_active_chat(chat_id):
            ...
            if not forceplay:
                db[chat_id] = []
            await Fubuki.join_call(chat_id, original_chat_id, file_path, video=status)
            await put_queue(
                chat_id,
                original_chat_id,
                file_path,
