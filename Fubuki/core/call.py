...
from Fubuki import LOGGER, Platform, app, userbot
...
from Fubuki.utils.exceptions import AssistantErr
...
from Fubuki.core.userbot import assistants
from Fubuki.utils.database import (
    get_assistant,
    set_assistant,
)
...
class Call:
        ...
        if join_as is None:
            try:
                ...
                LOGGER(__name__).warning(
                    f"Could not resolve assistant peer for stream check: {type(exc).__name__}: {exc}"
                )
    ...
    async def join_call(
        ...
        try:
            ...
            raise AssistantErr(
                "**No Active Voice Chat Found**\n\nPlease make sure group's voice chat is enabled. If already enabled, end it and start a fresh voice chat."
            )
        except AlreadyJoinedError:
            raise AssistantErr(
                "**ASSISTANT IS ALREADY IN VOICECHAT**\n\nAssistant is already in the voice chat. If the issue continues, restart the video chat."
            )
        except TelegramServerError:
            raise AssistantErr(
                "**TELEGRAM SERVER ERROR**\n\nPlease restart your voice chat."
            )
        except Exception:
            ...
            try:
                ...
                raise AssistantErr(f"**Error Joining Stream**: `{e}`")
