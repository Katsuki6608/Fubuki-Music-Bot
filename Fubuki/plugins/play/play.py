...
from Fubuki import app, LOGGER, Platform
from Fubuki.utils import seconds_to_min, time_to_seconds
from Fubuki.utils.database import is_video_allowed
from Fubuki.utils.decorators.play import PlayWrapper
from Fubuki.utils.formatters import formats
...
from Fubuki.utils.inline.playlist import botplaylist_markup
from Fubuki.utils.logger import play_logs
from Fubuki.utils.stream.stream import stream
from Fubuki.utils.notify import notify_owner
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        sp_checker = getattr(Platform, "spotify", None)
        ...
        elif sp_checker and await sp_checker.valid(url):
            sp_platform = True
            if not config.SPOTIFY_CLIENT_ID and not config.SPOTIFY_CLIENT_SECRET:
                return await mystic.edit_text(
                    "Service client keys are missing in config."
                )
            if "track" in url:
                try:
                    details, track_id = await sp_checker.track(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "youtube"
                img = details["thumb"]
                ...
                try:
                    details, plist_id = await sp_checker.playlist(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spplay"
                ...
                try:
                    details, plist_id = await sp_checker.album(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spalbum"
                ...
                try:
                    details, plist_id = await sp_checker.artist(url)
                except Exception:
                    return await mystic.edit_text(_["play_3"])
                streamtype = "playlist"
                plist_type = "spartist"
